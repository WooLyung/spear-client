...
public class Selected_skill1_4 extends GameObject {
    ...
    public void start() {
        ...
        transform.position.y = -20.48f + 2.2f;
    ...
    public void update() {
        ...
        for (int i = 0; i < 5; i++) {
            if (Input.getTouchState(i) == Input.TOUCH_STATE.DOWN) {
                if (Vector.distanceDouble(Input.getTouchWorldPos(i), transform.position) <= 130/147f * 130/147f) { // 버튼을 클릭했을 경우
                    ActionManager.skill1[3] = ActionManager.selectedSkill;
