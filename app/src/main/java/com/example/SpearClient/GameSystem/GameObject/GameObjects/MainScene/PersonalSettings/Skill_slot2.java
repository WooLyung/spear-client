...
public class Skill_slot2 extends GameObject {
        ...
        for (int i = 0; i < 5; i++) {
            if (Input.getTouchState(i) == Input.TOUCH_STATE.DOWN) {
                if (Vector.distanceDouble(Input.getTouchWorldPos(i), transform.position) <= 170/147f * 170/147f) { // 버튼을 클릭했을 경우
                    ...
                    ((MainScene)Game.engine.nowScene).skill_slot2.selected.setIsVisible(true);
                    ...
                    ((MainScene)Game.engine.nowScene).skillCard.spriteRenderer.bindingImage(GLRenderer.findImage("skillcard_deep_stab"));
                }
            }
        }
    }
