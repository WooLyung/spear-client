...
public class Skill_slot6 extends GameObject {
    ...
    public void start() {
        ...
        transform.position.y = -20.48f - 2.7f - 0.7f + 0.4f;
        transform.position.x = -(float) GLView.nowWidth + 6f - 1.1f;
        ...
        for (int i = 0; i < 5; i++) {
            if (Input.getTouchState(i) == Input.TOUCH_STATE.DOWN) {
                if (Vector.distanceDouble(Input.getTouchWorldPos(i), transform.position) <= 170/147f * 170/147f) { // 버튼을 클릭했을 경우
                    ...
                    ((MainScene)Game.engine.nowScene).skill_slot6.selected.setIsVisible(true);
                    ((MainScene)Game.engine.nowScene).skillCard.spriteRenderer.bindingImage(GLRenderer.findImage("skillcard_skim"));
                }
            }
        }
    }
