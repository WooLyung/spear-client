...
public class Skill_slot6 extends GameObject {
    ...
    public void start() {
        ...
        transform.position.y = -20.48f - 2.7f - 0.7f;
        transform.position.x = (float) GLView.nowWidth - 5.5f - 1.15f;
    ...
    public void update() {
        ...
        for (int i = 0; i < 5; i++) {
            if (Input.getTouchState(i) == Input.TOUCH_STATE.DOWN) {
                if (Vector.distanceDouble(Input.getTouchWorldPos(i), transform.position) <= 100/147f * 100/147f) { // 버튼을 클릭했을 경우
